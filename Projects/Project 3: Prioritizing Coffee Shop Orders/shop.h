...
class Shop{
     ...
     ******************************************/
    Order* copyHeap(Order* node);
    void clearHeap(Order* node);
    Order* mergeHeap(Order* p1, Order* p2);
    void reorganizeHeap();
    void collectNodes(Order* node, Order** arr, int& index);
    void printPreOrder(Order* node) const;
};
class Region{
    public:
    friend class Grader; // for grading purposes
